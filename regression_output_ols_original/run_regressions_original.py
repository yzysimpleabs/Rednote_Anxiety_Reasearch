...
def write_model_structure(f, model_title, y_col, y_desc, x_numeric_cols, x_categorical_cols, baselines):
    f.write("### 模型构成\n\n")
...
def main():
    ...
    df["ratio_collect"] = np.where(df["like"] > 0, df["collect"] / df["like"], np.nan)

    ces_total = df["like"] + df["collect"] + df["comments"] * 4 + df["share"] * 4
    df["ces_per_1000_view"] = np.where(df["view"] > 0, (ces_total / df["view"]) * 1000, 0.0)
    df["ln_ces_per_1000_view_plus1"] = safe_log1p(df["ces_per_1000_view"])

    df["ln_note_quote_plus1"] = safe_log1p(df["note_quote"])
    df["ln_view_plus1"] = safe_log1p(df["view"])
    ...
    save_resid_hist(e1, os.path.join(fig_dir, "residuals_model1.png"), "Residuals: Model1")
    ...
    save_resid_hist(e2, os.path.join(fig_dir, "residuals_model2.png"), "Residuals: Model2")
    ...
    save_resid_hist(e3, os.path.join(fig_dir, "residuals_model3.png"), "Residuals: Model3")
    ...
    md_path = os.path.join(out_dir, "regression_results_original.md")
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# 回归结果（原版：混合 OLS + White Robust / HC1；无缩尾、无拆分、无聚类）\n\n")
